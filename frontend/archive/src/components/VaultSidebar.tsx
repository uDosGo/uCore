...
import { FileEntry } from '../types/filepicker'
import { WorkspaceFilter } from './WorkspaceFilter'
import { BinderMissionFilter } from './BinderMissionFilter'

// ─── Types ──────────────────────────────────────────────────────────
// VaultFile is now unified with FileEntry - re-export for backward compatibility
export type VaultFile = FileEntry
...
const VaultSidebar: React.FC<VaultSidebarProps> = ({
  ...
  const [activeWorkspace, setActiveWorkspace] = useState<FileSource>('user')
  const binderRef = useRef<HTMLDivElement>(null)
  const isServerMode = sidebarMode === 'server'
            ...
            <>
              ...
              <BinderMissionFilter
                ...
                onBinderChange={(binderId) => {
                  const binder = availableBinders.find(b => b.id === binderId)
                  if (binder) {
