/* ═══════════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════════
   Tabs: Fallback, Tools, Services, Variables, Secrets, Global, User
   Secrets tab includes GitHub secrets sync (user UID-bound).
...
type SystemTab = 'pages' | 'tools' | 'services' | 'variables' | 'secrets' | 'global-settings' | 'user-settings'

const SYSTEM_TABS: SystemTab[] = ['pages', 'tools', 'services', 'variables', 'secrets', 'global-settings', 'user-settings']
    ...
    <div className="usx-surface-layout workflow-surface">
      ...
      <div className="usx-surface-body" style={{ position: 'relative' }}>
        ...
        <main className="usx-surface-main">
          {activeTab === 'pages' && !inlineSPage && <SystemPagesBrowser />}
          {activeTab === 'tools' && !inlineSPage && <SPageToolsPanel />}
          ...
          {activeTab === 'secrets' && (
            <div className="system-panel">
              <div className="system-panel-header">
                <h3><Icon name="key" size={16} /> User Secrets</h3>
                <span className="system-panel-count">UID-bound</span>
              </div>
              <SecretStorePanel />
            </div>
          )}
