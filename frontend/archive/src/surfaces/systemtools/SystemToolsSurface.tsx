...
import '../system/system-surface.css'
    ...
    <div className="system-panel">
      ...
      <div className="system-section">
        {categories.map(category => (
          <div key={category} className="system-section">
            <h4 className="system-section-title">
            ...
            <div className="system-card-grid">
              {SERVICES.filter(t => t.category === category).map(service => (
                <div 
                  key={service.id} 
                  className={`system-card ${service.status === 'active' ? '' : 'no-focus'}`}
                  role="button"
                  tabIndex={0}
                >
                  <div className="system-card-header">
                    <div className="system-card-icon" style={{ 
                      background: service.status === 'active' 
                        ? 'color-mix(in srgb, var(--pico-primary, #58a6ff) 15%, transparent)'
                        : 'var(--pico-card-sectioning-background-color, #1c2128)'
                    }}>
                      <Icon name={service.icon as any} size={20} />
                    </div>
                    <h5 className="system-card-title">{service.name}</h5>
                  </div>
                  <p className="system-card-desc">{service.description}</p>
                  <div className="system-card-footer">
                    <span className="system-card-code">{service.category}</span>
                    <span className={`system-card-status ${service.status === 'active' ? '' : 'system-card-status--stub'}`}>
                      {service.status === 'active' ? service.actionLabel : 'Planned'}
                    </span>
                  </div>
                </div>
              ))}
...
function SPageToolsPanel() {
  ...
  return (
    <div className="system-panel">
      ...
      <div className="system-card-grid">
        {S_PAGE_TOOLS.map(tool => (
          <div
            ...
            role="button"
            tabIndex={0}
            onClick={() => navigate(`/system?tab=tools&page=${tool.id}`)}
          >
            <div className="system-card-header">
              <div className="system-card-icon">
                <Icon name={tool.icon as any} size={20} />
              </div>
              <h5 className="system-card-title">{tool.name}</h5>
            </div>
            <p className="system-card-desc">{tool.description}</p>
            <div className="system-card-footer">
              <span className="system-card-code">{tool.route}</span>
              <span className={`system-card-status ${tool.status === 'built' ? '' : 'system-card-status--stub'}`}>
                {tool.status}
              </span>
            </div>
          </div>
      ...
      <div className="system-info-box" style={{ marginTop: '20px' }}>
        <p>
          <Icon name="info" size={16} /> Backend services managed in{' '}
          <a href="/server?tab=services" style={{ color: 'var(--pico-primary)' }}>
            /server → Services
          </a>
        </p>
      </div>
