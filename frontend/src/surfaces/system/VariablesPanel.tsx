/* ═══════════════════════════════════════════════════════════════════
   VariablesPanel — Variable Store + embedded Secret Store
   ═══════════════════════════════════════════════════════════════════
   Sub-tabs: User (profile), Global (system), Custom (key/value),
   Secrets (embedded SecretStorePanel).
   ═══════════════════════════════════════════════════════════════════ */
...
type VariableScope = 'user' | 'system' | 'custom' | 'secrets'
  ...
  return (
    <div className="sys-page-section" style={{ margin: '0', padding: '0' }}>
      ...
      <p className="sys-page-section-desc">
        ...
        <strong> Custom:</strong> User-defined key/value pairs. <strong>Secrets:</strong> Encrypted credential store.
      </p>
      ...
      <div style={{ padding: '8px 16px' }}>
        <div className="sys-var-scope-tabs">
          {(['user', 'system', 'custom', 'secrets'] as VariableScope[]).map(scope => (
            <button
              key={scope}
            ...
            >
              <Icon name={
                ...
                scope === 'custom' ? 'add_circle' : 'key'
              } size={14} />
              <span>
                {scope === 'user' ? 'User' :
                 scope === 'system' ? 'Global' :
                 scope === 'custom' ? 'Custom' : 'Secrets'}
              </span>
            </button>
          ))}
        </div>
      ...
      <div style={{ padding: '0 16px 16px' }}>
        {loading && activeScope !== 'secrets' ? (
          ...
          <>
            ...
            {activeScope === 'secrets' && <SecretStorePanel />}
          </>
        )}
      </div>
    </div>
