...
// surfaces module — extracted from UIHubManager.tsx
...
// ─── Surfaces hidden from Surfaces tab ──
const HIDDEN_FROM_SURFACES_TAB = ['browserui', 'devstudio']
...
const FALLBACK_REGISTRY: SurfaceDef[] = [
  ...
  { id: 'assistui',  name: 'AssistUI',       subtitle: 'Canonical AI Chat',         description: 'Full-page AI chat with streaming responses, model selection, conversation management, and multi-agent support.', port: 0, color: '#a855f7', icon: 'smart_toy',   status: 'stopped', cell: 'L100-AA10-0104-1', embedded: true, route: '/assistui' },
  ...
  { id: 'devstudio', name: 'DevStudio',      subtitle: 'Development Lane',          description: 'DevStudio development environment with dev-mode chat, repo browser, skill runner, and code review.', port: 0, color: '#f97583', icon: 'tune',       status: 'stopped', cell: 'L100-AA10-0109-1', embedded: true, route: '/devstudio' },
  { id: 'usystem',   name: 'uSystem',        subtitle: 'System Pages Hub',          description: 'System information, health dashboard, diagnostics, maintenance, logs, network, security, storage, and console pages.', port: 5199, color: '#00ff9d', icon: 'smart_display', status: 'stopped', cell: 'L100-AA10-0110-1', embedded: true, route: '/system' },
...
function InstallPanel() {
  ...
  useEffect(() => {
    ...
    async function probeAll() {
      ...
      try {
        const res = await fetch(`${SNACKBAR_API}/api/docker/ps`, { signal: AbortSignal.timeout(2000) })
        if (res.ok) {
          const data = await res.json()
        ...
        try {
          const res = await fetch(`${SNACKBAR_API}/api/exec`, {
            method: 'POST',
      ...
      // uConnect Mod — always local (part of DevStudio)
  ...
  const installableApps = [
    ...
    { id: 'vscode', name: 'VS Code', subtitle: 'Code Editor', icon: 'code', color: '#58a6ff', category: 'Editor', desc: 'Primary code editor for uDos development. Integrates with DevStudio surface for seamless development workflow.', url: 'https://code.visualstudio.com' },
    { id: 'git', name: 'Git', subtitle: 'Version Control', icon: 'commit', color: '#f0883e', category: 'Tools', desc: 'Distributed version control system. Required for all uDos repositories and DevStudio project management.', url: 'https://git-scm.com' },
  ...
  const browserMods = [
    { id: 'userjs', name: 'user.js Config', subtitle: 'Zen Browser Settings', icon: 'settings', color: '#6c63ff', category: 'Configuration', desc: 'Pre-configured about:config settings that transform Zen into a dedicated uConnect shell. Hides top bar, disables new tabs, configures sidebar, and optimises performance.', path: '~/Code/DevStudio/zen-config/user.js' },
    { id: 'userchrome', name: 'userChrome.css', subtitle: 'Browser Chrome Styling', icon: 'palette', color: '#f59e0b', category: 'Styling', desc: 'CSS that hides browser chrome elements (nav bar, tab bar, menu bar) and styles the sidebar with uConnect branding and workspace badges.', path: '~/Code/DevStudio/zen-config/userChrome.css' },
    { id: 'usercontent', name: 'userContent.css', subtitle: 'Page Content Styling', icon: 'web', color: '#22c55e', category: 'Styling', desc: 'CSS for styling web pages within the uConnect shell, including Web Reader integration and workspace-aware page overrides.', path: '~/Code/DevStudio/zen-config/userContent.css' },
    { id: 'sidebar', name: 'Vault Sidebar', subtitle: 'uConnect Navigation', icon: 'folder', color: '#58a6ff', category: 'Extension', desc: 'Zen Browser sidebar panel providing navigation to all vaults in the ~/Vaults/ container, system folders, and uConnect tools.', path: '~/Code/DevStudio/uconnect-mod/sidebar/sidebar.html' },
    { id: 'background', name: 'Background Script', subtitle: 'Tab & Protocol Handler', icon: 'sync', color: '#f0883e', category: 'Extension', desc: 'Background script that intercepts new tabs, handles udos:// protocol routing, manages context menus, and provides workspace detection.', path: '~/Code/DevStudio/uconnect-mod/background.js' },
    { id: 'content', name: 'Content Script', subtitle: 'Link Interception', icon: 'link', color: '#a855f7', category: 'Extension', desc: 'Content script that intercepts link clicks, prevents new tab/window openings, and routes udos:// URLs to the vault system.', path: '~/Code/DevStudio/uconnect-mod/content.js' },
  ]
  ...
  const syncDocs = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${SNACKBAR_API}/api/exec`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          command: `cd ~/Code/DevStudio && ./skills/docs-site-sync/run.sh 2>&1`,
        }),
  ...
  const docSections = [
    {
      ...
      docs: [
        { id: 'devstudio', name: 'DevStudio Guide', subtitle: 'Development environment setup & workflow', icon: 'code', url: 'http://localhost:5185/DevStudio/guide/devstudio-guide' },
        { id: 'workflow', name: 'Dev Workflow', subtitle: 'Daily development workflow & best practices', icon: 'route', url: 'http://localhost:5185/DevStudio/guide/dev-workflow' },
        { id: 'surfaces', name: 'Surfaces Guide', subtitle: 'Surface architecture & management', icon: 'dashboard', url: 'http://localhost:5185/DevStudio/guide/surfaces-guide' },
        { id: 'config', name: 'Configuration', subtitle: 'System configuration reference', icon: 'tune', url: 'http://localhost:5185/DevStudio/guide/configuration' },
        { id: 'user-guide', name: 'User Guide', subtitle: 'End-user documentation for uConnect', icon: 'person', url: 'http://localhost:5185/DevStudio/guide/user-guide' },
      ],
    },
    {
      id: 'references',
      ...
      docs: [
        { id: 'arch', name: 'Architecture', subtitle: 'System architecture overview', icon: 'account_tree', url: 'http://localhost:5185/DevStudio/guide/architecture-overview' },
        { id: 'roadmap', name: 'Roadmap', subtitle: 'Project roadmap & milestones', icon: 'map', url: 'http://localhost:5185/DevStudio/guide/roadmap-overview' },
        { id: 'changelog', name: 'Changelog', subtitle: 'Release history & changes', icon: 'history', url: 'http://localhost:5185/DevStudio/guide/changelog' },
        { id: 'narrator', name: 'Narrator System', subtitle: 'AI narration & logging system', icon: 'record_voice_over', url: 'http://localhost:5185/DevStudio/guide/narrator-system' },
        { id: 'skills', name: 'Skills Pipeline', subtitle: 'Automation skills system reference', icon: 'auto_awesome', url: 'http://localhost:5185/DevStudio/guide/skills-pipeline' },
      ],
    },
    {
      id: 'system',
      ...
      docs: [
        { id: 'snackbar', name: 'Snackbar API', subtitle: 'Surface management API reference', icon: 'restaurant_menu', url: 'http://localhost:5185/DevStudio/guide/snackbar-api' },
        { id: 'bin', name: 'CLI Tools', subtitle: 'Command-line tool reference', icon: 'terminal', url: 'http://localhost:5185/DevStudio/guide/cli-tools' },
        { id: 'scripts', name: 'Scripts', subtitle: 'Utility scripts reference', icon: 'script', url: 'http://localhost:5185/DevStudio/guide/scripts' },
        { id: 'extensions', name: 'Extensions', subtitle: 'Extension system documentation', icon: 'extension', url: 'http://localhost:5185/DevStudio/guide/extensions' },
        { id: 'modules', name: 'Modules', subtitle: 'Module system reference', icon: 'widgets', url: 'http://localhost:5185/DevStudio/guide/modules' },
      ],
    },
    {
      id: 'zen',
      ...
      docs: [
        { id: 'zen-setup', name: 'Setup Guide', subtitle: 'Install & configure Zen as uConnect shell', icon: 'auto_awesome', url: 'http://localhost:5185/DevStudio/guide/' },
        { id: 'zen-mod', name: 'uConnect Mod', subtitle: 'Zen Browser extension documentation', icon: 'puzzle', url: 'http://localhost:5185/DevStudio/guide/' },
        { id: 'zen-config', name: 'Configuration', subtitle: 'user.js & chrome CSS reference', icon: 'settings', url: 'http://localhost:5185/DevStudio/guide/configuration' },
        { id: 'zen-workspaces', name: 'Workspaces', subtitle: 'Workspace-to-vault mapping guide', icon: 'folder', url: 'http://localhost:5185/DevStudio/guide/' },
      ],
    },
  ]
    ...
    return (
      <div className="hub-docs" style={{ padding: 0, maxWidth: '100%', height: '100%', display: 'flex', flexDirection: 'column' }}>
        <div className="hub-docs-site-header">
          ...
          <span className="hub-docs-site-title">DevStudio Documentation Site</span>
        </div>
        <iframe
      ...
      <div className="hub-docs-section">
        <div className="hub-docs-section-header">
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <button
              ...
              title="Sync docs from DevStudio via Snackbar"
        ...
        <div className="hub-docs-grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))' }}>
          ...
          <div className="hub-doc-card" style={{ '--hub-doc-color': '#58a6ff', cursor: 'pointer' } as React.CSSProperties} onClick={() => setDocView('site')}>
            ...
            <p className="hub-install-card-desc">VitePress-powered documentation site with full-text search, navigation, and all DevStudio documentation.</p>
            <div className="hub-doc-card-footer">
...
function UIHubInner() {
  ...
  // devstudio is always hidden (merged into DevStudio Tools).
  const surfacesForTab = surfaces.filter(s => {
    // Hide devstudio — merged into DevStudio Tools
  ...
  const performAction = useCallback(async (id: string, action: SurfaceAction) => {
    ...
    try {
      ...
      switch (action) {
        case 'start':
          endpoint = `/api/surfaces/${id}/start`
          break
        case 'stop':
          endpoint = `/api/surfaces/${id}/stop`
          break
        case 'restart':
          endpoint = `/api/surfaces/${id}/restart`
          break
        case 'repair':
          endpoint = `/api/surfaces/${id}/repair`
          break
        case 'debug':
          endpoint = `/api/surfaces/${id}/debug`
          break
      }
