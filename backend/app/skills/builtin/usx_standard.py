...
USX_STANDARD_DIR = Path(__file__).parent.parent.parent.parent.parent / "frontend" / "src" / "styles"
USX_SURFACES_DIR = Path(__file__).parent.parent.parent.parent.parent / "frontend" / "src" / "surfaces"
USX_EXCLUDES = {"node_modules", ".git", "__pycache__"}

# Canonical USX files — the only source of truth for these classes
CANONICAL_FILES = {
    "usx-base.css": [".usx-surface-body", ".usx-surface-main", ".usx-header-btn", ".hub-status-badge", ".content-grid"],
...
class UsxStandardSkill(BaseSkill):
    meta = SkillMeta(
        ...
        description="Audit CSS for overrides/duplicates, repair to USX canonical standard",
        category="developer",
        params=[
            SkillParam(name="action", type="string",
                       description="Action: 'audit', 'repair', 'consolidate', 'report'",
                       required=True),
            SkillParam(name="target", type="string",
                       description="Optional: specific file or surface to target",
                       required=False),
            SkillParam(name="dry_run", type="bool",
    ...
    async def run(self, **kwargs) -> dict:
        ...
        if action == "audit":
            ...
            return {"success": True, "action": "consolidate", "message": "Extract shared patterns into usx-base.css"}
        elif action == "report":
            audit = self._deep_audit(target)
            return {"success": True, "action": "report", "report": audit,
    ...
    def _deep_audit(self, target: str = "") -> dict:
        ...
        findings = {
            "colors": [], "font_sizes": [], "icon_rules": [],
            "usx_duplicates": [], "surface_layout": [],
            "unscoped_headings": [], "redundant_imports": [],
            "inline_styles_tsx": [],
        }
        ...
        for css_file in css_files:
            ...
            if fonts:
                findings["font_sizes"].append({"file": rel, "issues": fonts[:10]})
                file_issues += len(fonts)

            # L3: Competing icon sizing
            icon_violations = self._check_icon_sizing(content, rel)
            if icon_violations:
                findings["icon_rules"].append({"file": rel, "issues": icon_violations})
    ...
    def _repair(self, target: str = "", dry_run: bool = False) -> dict:
        ...
        audit = self._deep_audit(target)
        repairs = {"colors_fixed": 0, "icon_rules_removed": 0,
                   "usx_dupes_removed": 0, "total_files_touched": 0}
        touched = set()
        ...
        for css_file in self._get_css_files(target):
            ...
            if new_content != content:
                repairs["colors_fixed"] += self._count_changes(content, new_content)
                content = new_content
                changed = True
        ...
        return {"success": True, "action": "repair", "dry_run": dry_run, "repairs": repairs}

    def _get_css_files(self, target: str = "") -> list[Path]:
        if not USX_STANDARD_DIR.exists():
            return []
        css_files = list(USX_STANDARD_DIR.rglob("*.css")) if not target \
            else list(USX_STANDARD_DIR.rglob(f"*{target}*"))
        return [f for f in css_files if not any(x in str(f) for x in USX_EXCLUDES)]
    ...
    def _check_hardcoded_colors(self, content: str) -> list[str]:
        ...
        for line in content.split('\n'):
            ...
            if '[data-theme' in stripped or '[data-palette' in stripped:
                in_var_block = True; continue
            if in_var_block and stripped == '}':
                in_var_block = False; continue
            if in_var_block: continue
            if '--' in stripped and ':' in stripped and 'var(' not in stripped: continue
            # Skip lines that already use var() — the hex is a fallback, not a hardcoded value
            if 'var(' in stripped: continue
            for match in re.findall(r"color:\s*#[0-9a-fA-F]{3,8}", stripped):
                issues.append(f"Hardcoded color: '{match}' → use var(--pico-*)")
                break
        return issues

    def _check_hardcoded_font_sizes(self, content: str, filename: str) -> list[str]:
    ...
    def _check_icon_sizing(self, content: str, rel: str) -> list[str]:
        ...
        if 'opsz' in content:
            issues.append("Has 'opsz' font-variation-setting — removes cap so icons scale with font-size")

        # Check for display: inline-block on icons (needs inline-flex for heading alignment)
        if 'display: inline-block' in content and '.material-symbols-outlined' in content:
            issues.append("Uses display: inline-block on icons — should be inline-flex for heading alignment")

        # Check for competing font-size rules on icons
        for match in re.finditer(r"\.material-symbols-outlined[\s\S]*?\{[^}]*font-size[^}]*\}",
                                  content):
            if 'font-size: 1em' not in match.group() and 'font-size: 1.5rem' not in match.group():
                lines = match.group().split('\n')
                for line in lines:
                    if 'font-size' in line:
                        issues.append(f"Competing icon font-size: '{line.strip()}' → remove, let usx-icons.css handle it")
        return issues

    def _check_usx_duplicates(self, content: str, rel: str) -> list[str]:
        """Find USX base class definitions outside canon files."""
        canon_files = ["usx-base.css", "usx-typography.css", "usx-icons.css"]
        ...
        for cls_list in CANONICAL_FILES.values():
            for cls in cls_list:
                ...
                for match in re.finditer(rf"{re.escape(cls)}\s*\{{", content):
                    issues.append(f"Duplicates USX class '{cls}' → import from usx-base.css instead")
        return issues
    ...
    def _check_surface_layout(self, content: str, rel: str) -> list[str]:
        ...
        for old_cls, new_cls in SURFACE_LAYOUT_MAP.items():
            if old_cls in content:
                issues.append(f"Uses '{old_cls}' instead of canonical '{new_cls}'")
        return issues

    def _check_unscoped_headings(self, content: str) -> list[str]:
        issues = []
        for h in ["h1", "h2", "h3", "h4", "h5", "h6"]:
            unscoped = re.findall(rf"(?:^|(?<={{))[\s]*{h}\s*\{{", content, re.MULTILINE)
            if unscoped:
                issues.append(f"Unscoped '{h}' selector → scope under a class")
        return issues
    ...
    def _fix_hardcoded_colors(self, content: str) -> str:
        pairs = [
            (r'color:\s*#ffffff', 'color: var(--pico-primary-inverse, #ffffff)'),
            (r'color:\s*#e6edf3', 'color: var(--pico-color, #e6edf3)'),
            (r'color:\s*#c9d1d9', 'color: var(--pico-color, #c9d1d9)'),
            (r'color:\s*#8b949e', 'color: var(--pico-muted-color, #8b949e)'),
            (r'color:\s*#d29922', 'color: var(--pico-warning-color, #d29922)'),
            (r'color:\s*#58a6ff', 'color: var(--pico-primary, #58a6ff)'),
            (r'color:\s*#3fb950', 'color: var(--pico-ins-color, #3fb950)'),
            (r'color:\s*#f85149', 'color: var(--pico-del-color, #f85149)'),
            (r'color:\s*#2f81f7', 'color: var(--pico-primary, #2f81f7)'),
            (r'color:\s*#9ba7b4', 'color: var(--pico-muted-color, #9ba7b4)'),
            (r'color:\s*#a855f7', 'color: var(--pico-primary, #a855f7)'),
        ...
        for i, line in enumerate(lines):
            s = line.strip()
            if '[data-theme' in s or '[data-palette' in s: in_var = True; continue
            if in_var and s == '}': in_var = False; continue
            if in_var: continue
            if '--' in s and ':' in s and 'var(' not in s: continue
            if 'var(' in s: continue
    ...
    def _recommendations(self, audit: dict) -> list[str]:
        recs = ["Use CSS variables (--pico-*) instead of hardcoded values",
                "Use `.prose` class for markdown content instead of per-surface prose styles",
                "Use USX layout classes from usx-base.css instead of surface-specific wrappers",
                "Strip .material-symbols-outlined font-size from all non-USX CSS files",
                "Remove duplicate USX class definitions from nestframe.css",
                "Do not redefine .usx-surface-body or .usx-surface-main in any surface CSS"]
        findings = audit.get("findings", {})
        for key, label in [("icon_rules", "Competing icon rules"),
            ...
            if findings.get(key):
                ...
                recs.append(f"Fix {label} in: {', '.join(sorted(files))}")
        return recs