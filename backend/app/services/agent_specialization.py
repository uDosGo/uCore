...
class SpecializedAgentRegistry:
    ...
    # ─── Cost Tier Mapping ─────────────────────────────────────
    # Maps agent cost thresholds to provider cost tiers.
    # Lower tier = cheaper; higher tier = more capable/expensive.
    ...
    def get_cost_tier_for_agent(
        ...
        Returns one of: free, ultra-cheap, budget, mid-range, premium
        """
        agent = self.agents.get(agent_id)
        if agent is None:
            return "mid-range"

        if agent.cost_per_task == 0.0:
            return "free"
        elif agent.cost_per_task <= 0.005:
            return "ultra-cheap"
        elif agent.cost_per_task <= 0.02:
            return "budget"
        elif agent.cost_per_task <= 0.10:
            return "mid-range"
        else:
            return "premium"
