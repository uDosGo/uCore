...
"""snacks module — extracted from mcp_bridge.py"""
...
class SnackmachineTransport:
    ...
    Writes commands to Snackmachine's JSONL queue file and polls for
    replies in the replies JSONL file. This is the primary transport
    for the Forge (Linux Mint) peer.
    """

    def __init__(self, config: dict):
        self.commands_path = Path(config.get("commands_path", "~/.local/share/snackmachine/commands.jsonl")).expanduser()
        self.replies_path = Path(config.get("replies_path", "~/.local/share/snackmachine/replies.jsonl")).expanduser()
    ...
    def write_command(self, cmd: dict) -> str:
        """Write a command to the Snackmachine queue file.

        Returns the correlation ID so the caller can match replies.
        """
    ...
    def poll_for_reply(self, correlation_id: str, timeout: float = None) -> Optional[dict]:
        """Poll the replies file for a matching correlation ID.

        Reads from the end of the file (most recent replies first)
        to find the matching reply efficiently.
        """
