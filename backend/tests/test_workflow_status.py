...
from app.services.workflow_status import build_workflow_status, scan_tasker_boards
...
def test_scan_tasker_boards(tmp_path: Path):
    ...
    (tasker_dir / "inbox" / "todo-first.md").write_text("# First\n", encoding="utf-8")
    (tasker_dir / "doing" / "doing-second.md").write_text("# Second\n", encoding="utf-8")
...
def test_build_workflow_status_includes_guardrails(tmp_path: Path, monkeypatch):
    ...
    result = build_workflow_status(maintenance={"status": "ok", "jobs": [{"skill_id": "brain_sync"}]})
